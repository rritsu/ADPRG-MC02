...
void RoomManager::generateRooms() { 
    this->generateRoomIndices();
    this->setCurrentRoom(this->nEntryRoom);
   // SceneManager::getInstance()->loadScene(this->getRoomTag(this->nEntryRoom));
    std::cout << "entryyy " << this->nEntryRoom << std::endl << std::endl;
}

void RoomManager::generateRoomIndices() {
...
void RoomManager::removeIndeces(std::vector<int> vecRemove) {
    ...
    bool bErase = false;
    for(int i = 0; i < vecRemove.size(); i++) {
        for(int j = 0; j < this->vecIndices.size(); j++) {
            if(vecRemove[i] == this->vecIndices[j] && bErase == false) {
                bErase = true;
                k = j;
            }
        }
        if(bErase == true) {
            this->vecIndices.erase(this->vecIndices.begin() + k);
        }
    ...
    std::cout << std::endl;
    std::cout << "size " << this->vecIndices.size() << std::endl;
    for(int i = 0; i < this->vecIndices.size(); i++) {
        std::cout << "room " << this->vecIndices[i] << std::endl;
    }
}
...
void RoomManager::instantiateRooms() {
    ...
    for(int i = 0; i < this->vecIndices.size(); i++) {
        int nRoomIndex = this->vecIndices[i];
    ...
    do{
        nTempIndex = Utility::getInstance()->getRandomNumber(0 , this->vecIndices.size() - 1);
    }while(this->vecIndices[nTempIndex] == 5);

    this->nEntryRoom = this->vecIndices[nTempIndex];
    Room* pRoom = this->findRoomByIndex(this->vecIndices[nTempIndex]);
...
void RoomManager::initializeIndices() {
    for(int i = 1; i <= 9; i++) {
        this->vecIndices.push_back(i);
    }
}

void RoomManager::clearIndices() {
    this->vecIndices.clear();
}
